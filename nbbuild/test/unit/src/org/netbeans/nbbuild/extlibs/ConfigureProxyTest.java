...
public class ConfigureProxyTest extends NbTestCase {
    ...
    public void testExecuteFailedConnection() throws MalformedURLException {
        ...
        assertEquals("", configureProxy.getProject().getUserProperty("http.proxyHost"));
        assertEquals("80", configureProxy.getProject().getUserProperty("http.proxyPort"));
    }
    
    public void testExecuteWithFailOnError() throws MalformedURLException {
        try {
            configureProxy.setFailOnError("true");
            configureProxy.execute();
